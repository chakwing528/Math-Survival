...
import { loadCloudConfig, DIFF_MULT } from './config.js?v=38';
import { initAudio, setBgmVolume, setSfxVolume } from './audio.js?v=38';
import { fetchLeaderboard, renderLeaderboard, submitScore } from './leaderboard.js?v=38';
import { TOPIC_NAMES } from './math.js?v=38';
import { initInputMode, initOrientationGuard, enterImmersive } from './device.js?v=38';

// 3D 引擎 (Three.js) 採用動態載入：就算 CDN 有問題，選單同排行榜都照常運作
let GameClass = null;
async function loadEngine() {
    if (!GameClass) {
        const mod = await import('./game.js?v=38');
        GameClass = mod.Game;
    }
    return GameClass;
}
...
async function startGame(level) {
    ...
    try {
        ...
        // 載入 3D 模型 (首次需時，顯示進度)
        const { loadAssets } = await import('./assets.js?v=38');
