...
- 畫面版本：Math Survival FPS V3.6。
- ES Module/cache query：`?v=36`，目前所有已加版本的 imports 一致。
...
- Issue #3 第一批已加入 `js/input.js`：keyboard/mouse/touch 共用 input state、純 lifecycle 狀態機和 listener cleanup。
- Pointer Lock 只負責 desktop control acquisition；`pause()`/`resume()`、數學題回復及 visibility pause 已與它解耦。
- touch 裝置已有暫停／回復按鈕；虛擬搖桿、touch look、射擊／換彈／近戰按鈕仍未完成。
...
- PR #1 的 V3.5 文件／安全基線仍未合併；V3.6 Issue #3 第一批在疊加獨立分支整理，避免直接改動 GitHub Pages。
...
- 2026-08-25：V3.6 static、18 個 unit tests及實際 touch 3D pause/resume browser 驗證通過；完整 `npm test`／CI 以分支最新結果為準。
...
3. 繼續 Issue #3 第二批：虛擬搖桿、touch look、開火／瞄準／換彈／近戰控制及真機驗收。
