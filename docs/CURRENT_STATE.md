...
- 畫面版本：Math Survival FPS V3.10。
- ES Module/cache query：`?v=40`，目前所有已加版本的 imports 一致。
...
- V3.10 touch HUD 只保留虛擬搖桿、touch look、按住開火及暫停；未持槍時開火鍵執行近戰，持槍後彈匣清空會自動換彈。desktop 右鍵瞄準、R 換彈及近戰行為保留。
...
- V3.10 手機效能方案：medium DPR 1.25／最低 1.0、45 FPS frame budget、同場最多 4 隻敵人；low 為 30 FPS／最多 3 隻。答題、暫停及結算時停止背景 effects 與重複 WebGL render；連續低 FPS 會降低 DPR、敵人上限及裝飾雲。`?debug=perf` 同時顯示敵人數／上限。
...
- V3.10 橫屏答題改為左側題目、右側答案／選項；答錯時右側改顯示解釋。Game Over 與 abort 同樣使用安全 Pointer Lock release，iPhone 缺少 API 仍會完成結算；上傳 exception 可重試。
...
- PR #1 的 V3.5 文件／安全基線仍未合併；V3.6 Issue #3 Batch 1 在 PR #5，V3.7 Batch 2 在 PR #7，自動裝置閘及 V3.8–V3.10 iPhone P0 修正位於 PR #8；四層都保持 Draft，避免直接改動 production GitHub Pages。
...
- Node unit tests：25 tests passed；包括 cloud/Supabase contract、input/lifecycle/touch/math-entry、Game Over 安全結算及自動換彈。
...
- Playwright browser matrix：desktop Chromium 6 個及三個 touch profiles 各 5 個，共 21 tests passed；2 個 hosted-Supabase cases 無 secrets 時按設計 skipped。涵蓋 3D/2D 啟動、雲端隔離、惡意排行榜、duplicate submission、手機 UI／畫質、答題包、Game Over 及 lifecycle。
